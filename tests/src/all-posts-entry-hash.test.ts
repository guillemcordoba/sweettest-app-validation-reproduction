...
test('create a Post and get all posts entry hash', async () => {
  await runScenario(async scenario => {
    const { alice, bob } = await setup(scenario);

    // Bob gets all posts entry hash
    let collectionOutput = await toPromise(bob.store.allPostsEntryHash);
    ...
    // Bob gets all posts entry hash again
    collectionOutput = await toPromise(bob.store.allPostsEntryHash);
