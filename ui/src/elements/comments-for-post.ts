...
/**
 * @element comments-for-post
 */
@localized()
@customElement('comments-for-post')
export class CommentsForPost extends LitElement {

  // REQUIRED. The PostHash for which the Comments should be fetched
  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0) 
      return html` <div class="column center-content" style="gap: 16px;">
        ...
        ></sl-icon>
        <span class="placeholder">${msg("No comments found for this post")}</span>
      </div>`;
  ...
  render() {
    return html`${subscribe(this.postsStore.posts.get(this.postHash).comments.live,
      renderAsyncStatus({
