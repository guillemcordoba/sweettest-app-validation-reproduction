...
/**
 * @element certificates-for-certified
 */
@localized()
@customElement('certificates-for-certified')
export class CertificatesForCertified extends LitElement {

  // REQUIRED. The Certified for which the Certificates should be fetched
  @property(hashProperty('certified'))
  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0) 
      return html` <div class="column center-content" style="gap: 16px;">
        ...
        ></sl-icon>
        <span class="placeholder">${msg("No certificates found for this certified")}</span>
      </div>`;
  ...
  render() {
    return html`${subscribe(this.postsStore.certificatesForCertified.get(this.certified),
      renderAsyncStatus({
