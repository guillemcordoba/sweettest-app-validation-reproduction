...
/**
 * @element posts-by-author-entry-hash
 */
@localized()
@customElement('posts-by-author-entry-hash')
export class PostsByAuthorEntryHash extends LitElement {
  ...
  firstUpdated() {
    if (this.author === undefined) {
      throw new Error(`The author property is required for the PostsByAuthorEntryHash element`);
    }
  }
  ...
  render() {
    return html`${subscribe(this.postsStore.postsByAuthorEntryHash.get(this.author),
      renderAsyncStatus({
