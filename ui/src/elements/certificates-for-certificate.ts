...
/**
 * @element certificates-for-certificate
 */
@localized()
@customElement('certificates-for-certificate')
export class CertificatesForCertificate extends LitElement {

  // REQUIRED. The CertificateHash for which the Certificates should be fetched
  @property(hashProperty('certificate-hash'))
  certificateHash!: EntryHash;
  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0) 
      return html` <div class="column center-content" style="gap: 16px;">
        ...
        ></sl-icon>
        <span class="placeholder">${msg("No certificates found for this certificate")}</span>
      </div>`;
  ...
  render() {
    return html`${subscribe(pipe(this.postsStore.certificates.get(this.certificateHash).certificates, hashes => slice(this.postsStore.certificates, hashes)),
      renderAsyncStatus({
