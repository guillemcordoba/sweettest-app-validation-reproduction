...
use posts::comment::UpdateCommentInput;
...
async fn create_and_read_comment() {
    ...
    let get_record: Option<Record> = conductors[1]
        .call(&bob_zome, "get_original_comment", record.signed_action.action_address().clone())
        .await;
...
#[tokio::test(flavor = "multi_thread")]
async fn create_and_update_comment() {
    // Use prebuilt dna file
    let dna_path = std::env::current_dir()
        .unwrap()
        .join(std::env::var("DNA_PATH").expect("DNA_PATH not set, must be run using nix flake check"));
    ...
    let sample_2 = sample_comment_2(&conductors[0], &alice_zome).await;
    let input = UpdateCommentInput {
      previous_comment_hash: original_action_hash.clone(),
      updated_comment: sample_2.clone(),
    };
    
    // Alice updates the Comment
    let update_record: Record = conductors[0]
        .call(&alice_zome, "update_comment", input)
        .await;
        
    let entry: Comment = update_record.entry().to_app_option().unwrap().unwrap();
    ...
    let get_record: Option<Record> = conductors[1]
        .call(&bob_zome, "get_latest_comment", original_action_hash.clone())
        .await;
  
    assert_eq!(update_record, get_record.unwrap());
    
    let input = UpdateCommentInput {
      previous_comment_hash: update_record.signed_action.hashed.hash.clone(),
      updated_comment: sample_1.clone(),
    };
    ...
    let all_revisions: Vec<Record> = conductors[1]
        .call(&bob_zome, "get_all_revisions_for_comment", original_action_hash.clone())
        .await;
